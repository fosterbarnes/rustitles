...
if ($Help) { Write-Host 'copyToDebian.ps1 [destination]'; return }
. (Join-Path $PSScriptRoot 'scriptHelper.ps1')
Set-Location -LiteralPath $repoRoot

$projectFolder = Split-Path -Path $repoRoot -Leaf
if (-not $Destination) {
    if (-not $env:GITHUB_DEBIAN_COPY_ROOT) {
        throw 'Debian destination is not configured. Pass -Destination or load the user profile.'
    }
    $Destination = Join-Path $env:GITHUB_DEBIAN_COPY_ROOT $projectFolder
}

if (-not (Test-Path -LiteralPath $Destination -PathType Container)) {
    $parent = Split-Path -Path $Destination -Parent
    if (-not (Test-Path -LiteralPath $parent -PathType Container)) {
        throw "Debian destination is not reachable: $Destination"
    }
}
